...
//    }
    ...
    protected void touchMove(MotionEvent event) {
        float xDiff = event.getX() - lastTouchX;
        float yDiff = event.getY() - lastTouchY;
        ...
        if (mLayout.verticalScrollEnabled()) {
            if (yDiff > 0 && viewPortY == 0) {
                ...
                //                    return;
                //no more scroll vertical, so if also no scroll hor, go away
                if (!mLayout.horizontalScrollEnabled()) {
                    return;
                }
            }
        ...
        if (mLayout.horizontalScrollEnabled()) {
            if (xDiff > 0 && viewPortX == 0) {
                if (mEdgeEffectsEnabled) {
                    ...
                    invalidate();
                    //no more scroll horizontal, so if also no scroll hor, go away
                    if (!mLayout.verticalScrollEnabled()) {
                        return;
                    }
                }
        ...
        if (mTouchMode == TOUCH_MODE_SCROLL) {
            moveViewportBy(xDiff, yDiff, false);
            lastTouchX = event.getX();
            lastTouchY = event.getY();
        }
    }
        ...
        public void run() {
            ...
            if (view != null) {
                performItemClick(view, beginTouchAt.itemSection,
                        beginTouchAt.itemIndex, mAdapter.getItemId(beginTouchAt.itemSection, beginTouchAt.itemIndex));
            }
            // }
        }
    }
